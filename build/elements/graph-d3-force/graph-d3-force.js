...
import workerize from "../../helper/workerize.js";
...
import requestAnimationFunction from "//cdn.jsdelivr.net/npm/requestanimationfunction/requestAnimationFunction.js"; // web worker same origin policy requires host to support OPTIONS CORS
...
export default class GraphD3Force extends GraphAddon {
  constructor(...args) {
    ...
    let _adaptive_links; // define own properties


    Object.defineProperties(this, {
      worker: {
        value: workerize(`const worker_url="${worker_url}";importScripts(worker_url);
  ...
  async __applyGraphUpdate(buffer) {
    if (!this.__graphChanged) {
      ...
      if (this.adaptiveLinks && !this.__linksHidden) {
        ...
        const time_difference = await requestTimeDifference(); // console.timeEnd("paint time");
...
_defineProperty(GraphD3Force, "tagName", "graph-d3-force");

_defineProperty(GraphD3Force, "defaultConfiguration", {
  link: {
    distance: 20,
    strength: .4
  },
