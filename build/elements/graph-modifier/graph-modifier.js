...
export default class GraphModifier extends GraphAddon {
  ...
  __bindNodes(host) {
    ...
    for (const [key, node] of host.nodes) {
      ...
      if (!node.modifierInstalled) {
        node.modifierInstalled = true; // node.hammer.options.domEvents = true;

        node.hammer.get("press").set({
          time: 600
        });
        node.hammer.on("press", this.__pressNode.bind(this, host, node)); // TODO: no stopImmediatePropagation available in hammer.js
        // e.g. detail-view tap is still triggered

        node.__tapHandlers = node.hammer.handlers.tap || []; // console.log("tap handlers", node.__tapHandlers);
  ...
  __pressCanvas(host, event) {
    if ((event.pointerType == "mouse" || this.touchPress) && event.srcEvent.path[0] === host.svg) {
      console.log(event); // @IMPORTANT: ensure new vertex key
      ...
      host.graph.addNewVertex(i, new host.Node({
        value: {},
        key: i
      }, host.__requestPaintNode.bind(host))); // this.dispatchEvent(new Event("graph-structure-change", {}));

      host.graph = host.graph; // get freshly created node
  ...
  __tapNode(host, node, event) {
    console.log(event); // @WORKAROUND: invoke usual listeners when not in modifying mode
...
_defineProperty(GraphModifier, "tagName", "graph-modifier");

_defineProperty(GraphModifier, "styleElement", style);
