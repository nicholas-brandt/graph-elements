...
export default class GraphTracker extends GraphAddon {
  ...
  async __onNodePaint(node, event) {
    if (this.trackingMode == "adaptive" && !node.__linksHidden) {
      ...
      node.__trackingTime = (node.__trackingTime * (this.trackingCount - 1) + time_difference) / this.trackingCount; // console.timeEnd("timediff");

      console.log("time difference", time_difference); // console.log("graph-tracker: node.__trackingTime", node.__trackingTime);
...
_defineProperty(GraphTracker, "tagName", "graph-tracker");
