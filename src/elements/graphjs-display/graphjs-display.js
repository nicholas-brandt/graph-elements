...
Polymer({
    ...
    _graphChanged(new_graph = this.graph, old_graph) {
        if (old_graph)
            for (let [node, relations] of old_graph)
        ...
        for (let link of this.graph.links) {
            ...
            if (link.source === link.target) link._path.setAttribute("loop", "");
            else link._path.removeAttribute("loop");
    ...
    _nodesUpdated() {
        ...
        for (let [node, {_circle}] of this.graph) {
            ...
            if (node.selected) _circle.setAttribute("selected", "");
            else _circle.removeAttribute("selected");
        }
