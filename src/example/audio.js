...
processor.onaudioprocess = event => {
    ...
    for (let channel = 0; channel < input_buffer.numberOfChannels; ++channel) {
        const input_data = input_buffer.getChannelData(channel);
        // let sum = 0;
        let square_sum = 0;
...
function processRMS(rms) {
    ...
    datasets[2].data = rms_series.__averageSeries.map((value, i) => ({x: i, y: value + z_margin_series[i] * rms_series.__deviationSeries[i]}));
    datasets[3].data = rms_series.__averageSeries.map((value, i) => ({x: i, y: Math.max(value - z_margin_series[i] * rms_series.__deviationSeries[i], 0)}));
