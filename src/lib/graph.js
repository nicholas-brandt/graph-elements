        ...
        const search = (node, path) => {
            ...
            for (let [neighbor_node] of this.get(node).dependents)
                if (path.length > max_length) {
                    ...
                    if ((this.directed || index != 2) && index <= path.length && index > max_length) max_length = index;
                    else if (index > path.length) search(neighbor_node, path.concat(neighbor_node));
                } else if (!path.includes(neighbor_node)) search(neighbor_node, path.concat(neighbor_node));
        };
        for (let [current_node] of this)
