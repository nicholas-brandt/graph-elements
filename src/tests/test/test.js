...
const static_edges = [
    for (node of nodes)[Math.floor(Math.abs(Math.sin(node)) * (length - 1)), Math.floor(Math.abs(Math.cos(node)) * (length - 1))]
];
...
for (let name of ["Graph", "AcyclicGraph", "Tree"])
    for (let edges of edge_array) {
        const graph = new window[name];
        const dgraph = new window[name](true);
